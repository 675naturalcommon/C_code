...
// 算法函数类型定义
typedef void (*AlgorithmFunc)(const DiskRequest *req, int sequence[], int moves[], int *count);
...
void printAccessChain(const int sequence[], const int moves[], int count, int head);
void sortRequests(const int requests[], int n, int sortedRequests[]);

// 算法函数声明
void fcfs(const DiskRequest *req, int sequence[], int moves[], int *count);
void sstf(const DiskRequest *req, int sequence[], int moves[], int *count);
void scan(const DiskRequest *req, int sequence[], int moves[], int *count, int direction);
void cscan(const DiskRequest *req, int sequence[], int moves[], int *count, int direction);
void look(const DiskRequest *req, int sequence[], int moves[], int *count, int direction);
void clook(const DiskRequest *req, int sequence[], int moves[], int *count, int direction);
...
void runDiskSchedulingAlgorithm(const DiskRequest *req) {
    ...
    struct {
        const char *name;
        AlgorithmFunc func;
        int direction;  // 0: 无方向, 1: 增加方向, 2: 减少方向
    } algorithms[] = {
        {"先来先服务算法 (FCFS)", (AlgorithmFunc)fcfs, 0},
        {"最短寻道时间优先算法 (SSTF)", (AlgorithmFunc)sstf, 0},
        {"扫描算法 (SCAN) - 向磁道号增加方向移动", NULL, 1},
        {"扫描算法 (SCAN) - 向磁道号减少方向移动", NULL, 2},
        {"循环扫描算法 (CSCAN) - 向磁道号增加方向移动", NULL, 1},
        {"循环扫描算法 (CSCAN) - 向磁道号减少方向移动", NULL, 2},
        {"LOOK算法 (SCAN改良版) - 向磁道号增加方向移动", NULL, 1},
        {"LOOK算法 (SCAN改良版) - 向磁道号减少方向移动", NULL, 2},
        {"CLOOK算法 (CSCAN改良版) - 向磁道号增加方向移动", NULL, 1},
        {"CLOOK算法 (CSCAN改良版) - 向磁道号减少方向移动", NULL, 2},
    };
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (i == 0) {
            fcfs(req, sequence, moves, &count);
        } else if (i == 1) {
            sstf(req, sequence, moves, &count);
        } else if (i == 2 || i == 3) {
            scan(req, sequence, moves, &count, i == 2 ? 1 : 2);
        } else if (i == 4 || i == 5) {
            cscan(req, sequence, moves, &count, i == 4 ? 1 : 2);
        } else if (i == 6 || i == 7) {
            look(req, sequence, moves, &count, i == 6 ? 1 : 2);
        } else if (i == 8 || i == 9) {
            clook(req, sequence, moves, &count, i == 8 ? 1 : 2);
        }

        printAccessChain(sequence, moves, count, req->head);
    }
...
void fcfs(const DiskRequest *req, int sequence[], int moves[], int *count) {
    ...
    *count = req->n;
    ...
    for (int i = 0; i < req->n; i++) {
        moves[i] = abs(current - req->requests[i]);
        sequence[i] = req->requests[i];
...
void sstf(const DiskRequest *req, int sequence[], int moves[], int *count) {
    ...
    *count = 0;
    ...
    while (*count < req->n) {
        ...
        sequence[*count] = req->requests[minIndex];
