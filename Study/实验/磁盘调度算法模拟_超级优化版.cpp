...
// 算法函数类型定义
typedef void (*AlgorithmFunc)(const DiskRequest *req, int *sequence, int *moves, int *count, int direction);
...
void runDiskSchedulingAlgorithm(const DiskRequest *req) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        algorithms[i].func(&tempReq, sequence, moves, &count, algorithms[i].direction);
